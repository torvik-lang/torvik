...
| security | 1 | `check_cmdline` reads the compiler's own source and fails if a value reaches a command line without a validator. Added after the seventh command-injection issue: a runtime check cannot stop a mistake made while *writing* the compiler. |
| fuzz | 1 | `fuzz` mutates real programs and asserts one property — for any input, torvc either compiles it or refuses cleanly. An internal error or a crash on text somebody typed is a bug. The negative cases test the mistakes we thought of; this tests the ones we did not. |

Every case is end-to-end: compile a real program with a real `torvc`, run it, and
compare what came out.

**The counts above go stale.** They are a description, not a gate — `run_tests`
prints the real total on every run, and that number is the one to trust. If they
